...
/// so this non-generic version requires one of the PolecatOps.StartStream&lt;T&gt;(Guid streamId, ...)
    ...
    public static void ConfigureResponse(IChain chain)
    {
        ...
        if (streams.Length == 0)
        {
            throw new InvalidOperationException(
                $"{nameof(CreatedAggregate)} cannot be used because Chain {chain} does not also return an {nameof(IStartStream)} value. Return one with PolecatOps.StartStream<T>()");
        }
        ...
        {
            throw new InvalidOperationException(
                $"{nameof(CreatedAggregate)} cannot determine the aggregate type from the {nameof(IStartStream)} return value of Chain {chain}. Either use one of the PolecatOps.StartStream<T>(Guid streamId, ...) overloads that return StartStream<T>, or use the generic CreatedAggregate<T> instead.");
        }
...
public class CreatedAggregate<T> : IResponseAware, ICreationAware where T : class, new()
{
    public CreatedAggregate()
    {
    }
    ...
    public static void ConfigureResponse(IChain chain)
    {
        ...
        {
            throw new InvalidOperationException(
                $"CreatedAggregate<{typeof(T).Name}> cannot be used because Chain {chain} does not also return an {nameof(IStartStream)} value. Return one with PolecatOps.StartStream<{typeof(T).Name}>()");
        }
    ...
    {
        // A creation chain has no [AggregateHandler]/[Aggregate] usage to add the Polecat
        ...
        // here (idempotently, mirroring PolecatPersistenceFrameProvider.ApplyTransactionSupport)
        // so the response fetch lands after the commit.
    ...
    {
        // Only the PolecatOps.StartStream<T>(Guid streamId, ...) overloads declare the
