...
{
    /// <param name="stream">The stream-start operation, e.g. from MartenOps.StartStream&lt;T&gt;(...)</param>
    ...
    public static void ConfigureResponse(IChain chain)
    {
        ...
        // MartenOpPolicy routes message handler chains that return IMartenOp values through
        // MartenPersistenceFrameProvider.ApplyTransactionSupport, but the wrapped StartStream<T>
        ...
        // lands after the commit. Known divergence from ApplyTransactionSupport: its optional
        // OTel bracketing of SaveChanges (Tracking.OutboxDiagnosticsEnabled) needs the
        // IServiceContainer this hook does not receive, so these chains skip it.
