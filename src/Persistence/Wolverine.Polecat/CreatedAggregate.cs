...
public class CreatedAggregate<T> : IResponseAware, ICreationAware where T : class
{
    ...
    {
        // PolecatOpPolicy already routes message handler chains that return a single IPolecatOp
        // through PolecatPersistenceFrameProvider.ApplyTransactionSupport, but Wolverine.Http has
    ...
    {
        // Only the PolecatOps.StartStream<T>(Guid streamId, ...) overloads declare the concrete
